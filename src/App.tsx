  ...
  if (!grid && profile) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-slate-900 text-white p-6 text-center">
        ...
        <h2 className="text-xl font-bold mb-2">Game Status: Ready</h2>
        
        {room?.hostId === profile.id && (
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 max-w-xs shadow-2xl">
            <p className="text-xs text-slate-400 uppercase tracking-widest mb-2 font-bold">Room ID</p>
            <div className="bg-black p-3 rounded-lg mb-2">
               <code className="text-pink-400 font-mono text-sm break-all">{room.code || 'Generating...'}</code>
            </div>
            <p className="text-[10px] text-slate-500 italic">Share this code with the other player.</p>
          </div>
        )}
      </div>
    );
